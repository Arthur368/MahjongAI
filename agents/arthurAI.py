...
from client.mahjong_patition import Partition
...
class ArthurAIv1(AIInterface):

    def t_34_to_matrix(self, tiles34):
    ...
    def waiting_card_calc(self, hand, meld, pre_shanten):

        total_waiting = 0
        for i in range(34):
            hand.append(i)
            hand = sorted(hand)
            if Partition.shantin_normal(hand, meld) > pre_shanten:                
                total_waiting += 1
        print("Finish one card!")
    ...
    def to_discard_tile(self):
        ...
        tiles34 = [t//4 for t in self.tiles136]
        meld34 = [t//4 for t in self.meld136]

        discard = self.tiles136[0]
        max_waiting = 0
        shanten = Partition.shantin_normal(tiles34, meld34)

        for t in tiles34:    
            tiles34cpy = copy.deepcopy(tiles34)
            tiles34cpy.remove(t)
            waiting = self.waiting_card_calc(tiles34cpy, meld34, shanten)
            if waiting > max_waiting:
                discard = 4*t
                max_waiting = waiting

        l = len(self.tiles136)
        for i in range(discard, discard + 4):
            if i in self.tiles136:
                return i

        return self.tiles136[l - 1]
